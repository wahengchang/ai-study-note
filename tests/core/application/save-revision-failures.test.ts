...
import type { DomainApplicationDependencies } from "../../../core/application/index.js";
...
function request(overrides: Partial<Parameters<ReturnType<typeof createDomainApplication>["saveRevision"]>[0]> = {}) {
  return {
...
test("a route already claimed by another entry fails with ROUTE_CONFLICT and leaves canonical state unchanged", () => {
  withStore("save-conflict-", (store) => {
    const app = createDomainApplication({ persistence: store, siteDefinition: createSiteDefinition({ persistence: store }), dataMedia: noMedia, schemaValidator: acceptEverySchema });
    assert.equal(app.saveRevision(request()).ok, true);
    const before = digestOf(store);
    const conflicted = app.saveRevision(request({ entryId: "entry-b", revisionId: "draft-2", operationId: "save-2" }));
...
test("media that disappears after the preflight keeps MEDIA_UNAVAILABLE instead of collapsing to SAVE_REVISION_FAILED", () => {
  withStore("save-media-", (store) => {
    // preflight 通過但 asset version 不在 canonical state：只有 transaction 內的檢查會攔下。
    const app = createDomainApplication({ persistence: store, siteDefinition: createSiteDefinition({ persistence: store }), dataMedia: noMedia, schemaValidator: acceptEverySchema });
    const before = digestOf(store);
    const saved = app.saveRevision(request({ assetVersions: [{ assetId: "asset-1", assetVersionId: "version-1" }] }));
...
test("a route graph that moves under the proposal keeps STALE_ROUTE_PROPOSAL", () => {
  withStore("save-stale-", (store) => {
    ...
    const app = createDomainApplication({ persistence: store, siteDefinition: staleSite, dataMedia: noMedia, schemaValidator: acceptEverySchema });
    const before = digestOf(store);
    const saved = app.saveRevision(request());
    assert.equal(saved.ok, false);
    if (saved.ok) return;
...
test("an unregistered schema version is reported as a correctable request", () => {
  withStore("save-schema-", (store) => {
    const app = createDomainApplication({ persistence: store, siteDefinition: createSiteDefinition({ persistence: store }), dataMedia: noMedia, schemaValidator: acceptEverySchema });
    const before = digestOf(store);
    const saved = app.saveRevision(request({ schemaIdentity: { schemaId: "note", version: 9 } }));
    assert.equal(saved.ok, false);
    if (saved.ok) return;
...
test("SaveRevision moves only the current pointer and preserves the published pointer", () => {
  withStore("save-published-", (store) => {
    const app = createDomainApplication({ persistence: store, siteDefinition: createSiteDefinition({ persistence: store }), dataMedia: noMedia, schemaValidator: acceptEverySchema });
    const first = app.saveRevision(request());
    ...
    const second = app.saveRevision(request({ revisionId: "draft-2", operationId: "save-2" }));
