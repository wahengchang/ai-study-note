...
async function harness(directory: string): Promise<Harness> {
  ...
  const application = createDomainApplication({ persistence: opened.value, siteDefinition: createSiteDefinition({ persistence: opened.value }), dataMedia: media, schemaValidator: { validate: () => ({ ok: true }) }, pluginHost: plugins.value, taxonomy: createTaxonomy({ persistence: opened.value }) });
...
test("RestoreRevision creates a new current immutable revision and retains the published pin", async () => {
  ...
  try {
    ...
    const saved = await application.saveRevision({ entryId: "entry", revisionId: "draft", operationId: "save-1", expectedCurrentRevisionId: null, schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "old" }, route: "/old", assetVersions: [{ assetId: "asset", assetVersionId: "v1" }], taxonomyTerms: [] });
    ...
    assert.equal((await application.saveRevision({ entryId: "entry", revisionId: "draft-2", operationId: "save-2", expectedCurrentRevisionId: "draft", schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "new" }, route: "/old", assetVersions: [], taxonomyTerms: [] })).ok, true);
...
test("RestoreRevision refuses unavailable media before any mutation and succeeds once RestoreAsset completes", async () => {
  ...
  try {
    ...
    assert.equal((await application.saveRevision({ entryId: "entry", revisionId: "draft", operationId: "save-1", expectedCurrentRevisionId: null, schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "old" }, route: "/old", assetVersions: [{ assetId: "asset", assetVersionId: "v1" }], taxonomyTerms: [] })).ok, true);
    assert.equal((await application.saveRevision({ entryId: "entry", revisionId: "draft-2", operationId: "save-2", expectedCurrentRevisionId: "draft", schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "new" }, route: "/old", assetVersions: [{ assetId: "asset", assetVersionId: "v2" }], taxonomyTerms: [] })).ok, true);
...
test("RestoreRevision rejects malformed requests and unknown source revisions without touching canonical state", async () => {
  ...
  try {
    ...
    assert.equal((await application.saveRevision({ entryId: "entry", revisionId: "draft", operationId: "save-1", expectedCurrentRevisionId: null, schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "old" }, route: "/old", assetVersions: [], taxonomyTerms: [] })).ok, true);
    const before = store.canonicalState();
    assert.equal(before.ok, true);
    if (!before.ok) return;
