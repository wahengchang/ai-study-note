...
test("SaveRevision atomically creates current revision, pointer, and claim", () => {
  ...
  try {
    ...
    const app = createDomainApplication({ persistence: opened.value, siteDefinition: site, dataMedia: media, schemaValidator: { validate: () => ({ ok: true }) } });
    const saved = app.saveRevision({ entryId: "entry", revisionId: "draft-1", operationId: "save-1", schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "draft" }, route: "/Learn//Guide/", assetVersions: [] });
    assert.equal(saved.ok, true, saved.ok ? "" : saved.error.code);
    if (!saved.ok) return;
    assert.equal(saved.value.currentPointer.currentRevisionId, "draft-1");
    assert.equal(saved.value.currentClaim.normalizedRoute, "/learn/guide");
    opened.value.close();
