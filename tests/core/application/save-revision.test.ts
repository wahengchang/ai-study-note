...
test("SaveRevision atomically creates current revision, pointer, and claim", async () => {
  ...
  try {
    ...
    const media = createDataMedia({
      persistence: opened.value,
      objectStore: {
        stage: () => ({ ok: false, error: { code: "MEDIA_STAGING_FAILURE", owner: "DataMedia", subjectIds: [], remediation: { kind: "message", message: "" } } }),
        promote: () => ({ ok: false, error: { code: "MEDIA_PROMOTION_FAILURE", owner: "DataMedia", subjectIds: [], remediation: { kind: "message", message: "" } } }),
        verifyFinal: () => ({ ok: true, value: undefined }),
        releaseStage: () => ({ ok: true, value: undefined }),
        verifyEvidence: () => ({ ok: true, value: undefined }),
        inspectFinal: () => ({ ok: true, value: "absent" }),
      },
    });
