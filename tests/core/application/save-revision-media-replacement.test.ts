...
test("SaveRevision media replacement copies the complete set, moves only current, and preserves published history", async () => {
  ...
  try {
    ...
    const app = createDomainApplication({ persistence: opened.value, siteDefinition: site, dataMedia: media, schemaValidator: { validate: () => ({ ok: true }) }, pluginHost: host.value, contentReadModel: contentReadModel() });
...
async function replacementFixture() {
  ...
  const site = createSiteDefinition({ persistence: opened.value });
  const dependencies: DomainApplicationDependencies = { persistence: opened.value, siteDefinition: site, dataMedia: media, schemaValidator: { validate: () => ({ ok: true }) }, pluginHost: host.value, contentReadModel: contentReadModel() };
