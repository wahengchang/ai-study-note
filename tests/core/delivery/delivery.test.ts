...
test("Re-delivery 拒絕未經 manifest 列舉的額外 bytes 與未驗證的 artifact 位址", () => {
  ...
  try {
    ...
    const built = delivery.value.deliver({ contract: "renderer-output/v1", rendererInputDigest: digest("input"), provenance: { publishedRevisionIds: [], routeGraphDigest: "sha256:routes", mediaSelectionDigest: "sha256:media", theme: { id: "theme", version: "1.0.0", manifestHash: "sha256:theme" }, plugins: [] }, routes: [{ route: "/guide", filePath: "guide/index.html" }], files: [{ path: "guide/index.html", bytes, digest: sha256Digest(bytes) }], outputDigest: digest("output") });
...
test("落敗的並行交付只回收自己的 staging，不刪除已存在的 digest 目錄", () => {
  ...
  try {
    ...
    if (!delivery.ok) return;
    const bytes = new TextEncoder().encode("<main>public</main>\n");
    const output = { contract: "renderer-output/v1" as const, rendererInputDigest: digest("input"), provenance: { publishedRevisionIds: [], routeGraphDigest: digest("routes"), mediaSelectionDigest: digest("media"), theme: { id: "theme", version: "1.0.0", manifestHash: digest("theme") }, plugins: [] }, routes: [{ route: "/guide", filePath: "guide/index.html" }], files: [{ path: "guide/index.html", bytes, digest: sha256Digest(bytes) }], outputDigest: digest("output") };
