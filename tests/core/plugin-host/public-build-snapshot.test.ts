...
async function fixture(indexing: "allow" | "disallow" = "allow") {
  ...
  const entry = new TextEncoder().encode("export function page(){return {contract:'public-seo-page-contribution/v1',contribution:{title:'Post'}}} export function site(input){return {contract:'public-seo-site-contribution/v1',contribution:{publicSiteUrl:input.settings.publicSiteUrl,indexing:input.settings.indexing}}}");
  ...
  const saved = await created.value.replaceSettings({ identity, expectedSettingsStateDigest: snapshot.value.stateDigest, settingsContract: "seo-plugin-settings/v1", settings: { contract: "seo-plugin-settings/v1", publicSiteUrl: "https://example.test/", indexing } }); if (!saved.ok) throw new Error(saved.error.code);
...
test("prepared public snapshot 保留 SEO indexing evidence", async () => {
  const value = await fixture("disallow");
  try {
    const prepared = await value.host.resolvePublicBuildSnapshot({ contract: "public-plugin-build-request/v1", published: { route: "/post/" } });
    assert.equal(prepared.ok, true);
    if (!prepared.ok || prepared.value.snapshot.seo.status !== "available") return;
    assert.deepEqual(prepared.value.snapshot.seo.site[0]?.contribution.contribution, { publicSiteUrl: "https://example.test/", indexing: "disallow" });
  } finally {
