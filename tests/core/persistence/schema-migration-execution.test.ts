...
test("executes one fresh approvable plan atomically and reconstructs durable lineage", () => {
  ...
  try {
    ...
    if (canonical.ok) assert.deepEqual(canonical.value.counts, { schemaVersions: 2, revisions: 2, operationLineage: 3, entryPointers: 1, entryPointerLineage: 3, routeClaims: 2, mediaImportIntents: 0, mediaObjects: 0, mediaAssets: 0, assetVersions: 0, revisionReferences: 0, taxonomies: 2, taxonomyTermIdentities: 0, taxonomyTerms: 0, revisionTaxonomyBindings: 0, currentContentTypes: 1, globalSlugClaims: 3, currentEntries: 0, pluginActivationStates: 1, themeActivationStates: 1, pluginSettingsStates: 1, schemaMigrationExecutions: 1, schemaMigrationRevisionLineage: 1, schemaMigrationPointerLineage: 2 });
