...
test("相同 canonical revision bytes 產生 byte-identical structured content artifact", () => {
  ...
  if (!model.ok) return;
  const revision = input({ contract: "site-content/v1", title: "公開筆記", blocks: [{ kind: "article", text: "內容" }], seo: {} });
  ...
  assert.deepEqual(first.value.content, { contract: "site-content/v1", title: "公開筆記", blocks: [{ kind: "article", text: "內容" }], seo: {} });
});
...
test("model 拒絕非 canonical 或 digest 不符的 revision bytes，不產生 partial content", () => {
  ...
  const valid = input({ contract: "site-content/v1", title: "x", blocks: [], seo: {} });
...
test("raw full-page 僅由明確核准 schema identity 讀取", () => {
  const raw = { contract: "site-content/v1", title: "核准原始頁", blocks: [{ kind: "raw-full-page", html: "<main>raw</main>", staticFallback: "raw fallback" }], seo: {} };
  const denied = createContentReadModel({ approvedRawFullPageSchemas: [] });
  ...
  const allowed = createContentReadModel({ approvedRawFullPageSchemas: [{ schemaId: "site-content", version: 1 }] });
...
test("model fail-closed 拒絕未知 block 與不完整 demo source", () => {
  ...
  assert.equal(model.ok, true);
  if (!model.ok) return;
  const unknown = model.value.read(input({ contract: "site-content/v1", title: "x", blocks: [{ kind: "unknown" }], seo: {} }));
  assert.equal(unknown.ok, false);
  if (!unknown.ok) assert.equal(unknown.error.code, "INVALID_STRUCTURED_CONTENT");
  const incompleteDemo = model.value.read(input({ contract: "site-content/v1", title: "x", blocks: [{ kind: "interactive-demo", pluginIdentity: demoIdentity, source: { html: "", css: "" }, staticFallback: "fallback" }], seo: {} }));
