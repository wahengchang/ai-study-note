...
test("renderer input stays published-only while current preview exposes only its subject draft", async () => {
  ...
  try {
    ...
    for (const [revisionId, content] of [["r1", { contract: "site-content/v1", title: "published", blocks: [{ kind: "article", text: "published" }], seo: {} }], ["r2", { contract: "site-content/v1", title: "DRAFT_SECRET", blocks: [{ kind: "article", text: "DRAFT_SECRET" }], seo: {} }]] as const) {
      ...
      assert.equal(store.createRevision({ identity: { entryId: "entry-a", revisionId }, schemaIdentity: { schemaId: "site-content", version: 1 }, contentBytes: bytes, contentDigest: sha256Digest(bytes), lineage: { operationId: `save-${revisionId}`, operationKind: "SaveRevision" } }).ok, true);
    }
