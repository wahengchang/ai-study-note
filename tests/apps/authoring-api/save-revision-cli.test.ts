...
test("SaveRevision CLI maps a failed save to exit code 1 with the failure code on stderr", async () => {
  ...
  const environment = { cwd: directory, homeDirectory: directory, xdgConfigHome: path.join(directory, "config") };
  try {
    writeFileSync(path.join(directory, "request.json"), JSON.stringify({ contract: "save-revision-request/v1", revisionId: "r", operationId: "o", expectedCurrentRevisionId: null, schemaIdentity: { schemaId: "note", version: 1 }, content: { title: "t" }, route: "/r", assetVersions: [], taxonomyTerms: [] }));
...
test("SaveRevision CLI rejects a request file that omits content", async () => {
  ...
  const environment = { cwd: directory, homeDirectory: directory, xdgConfigHome: path.join(directory, "config") };
  try {
    writeFileSync(path.join(directory, "request.json"), JSON.stringify({ contract: "save-revision-request/v1", revisionId: "r", operationId: "o", expectedCurrentRevisionId: null, schemaIdentity: { schemaId: "note", version: 1 }, route: "/r", assetVersions: [], taxonomyTerms: [] }));
