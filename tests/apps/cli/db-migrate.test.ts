...
test("CLI reports exact fresh and rerun migration output", () => {
  ...
  try {
    ...
    assert.equal(first.stdout, "DB_MIGRATE_OK applied=13 current=0013-add-current-entries\n");
    assert.equal(first.stderr, "");
    const rerun = invoke(["--database", databasePath]);
    assert.equal(rerun.status, 0);
    assert.equal(rerun.stdout, "DB_MIGRATE_OK applied=0 current=0013-add-current-entries\n");
