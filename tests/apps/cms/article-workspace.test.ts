...
test("authenticated CMS 完成 article-first history、save、preview 與 published-with-draft workflow", async () => {
  ...
  try {
    ...
    await page.getByRole("link", { name: "跳到內容", exact: true }).focus();
    ...
    await page.getByTitle("目前版本頁面預覽", { exact: true }).waitFor();
    ...
    await page.getByTitle("已發布版本頁面預覽", { exact: true }).waitFor();
