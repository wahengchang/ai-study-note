...
test("entry editor 以 local CMS browser/a11y gate 顯示 Host output，並保留所有 safe source", async () => {
  ...
  try {
    for (const status of ["active", "inactive", "missing", "identity-changed"] as const) {
      ...
      const server = createServer(async (request, response) => {
        ...
        if (pathname === "/cms/entries/entry-plugin") { response.writeHead(200, { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" }); response.end(`<!doctype html><html><body><div id="root"><main aria-busy="true"><h1>CMS 工作台載入中</h1></main></div><script type="module" src="/cms/${assets.bootstrapPath}"></script></body></html>`); return; }
      ...
      try {
        await page.goto(`http://127.0.0.1:${address.port}/cms/entries/entry-plugin`, { waitUntil: "networkidle" });
