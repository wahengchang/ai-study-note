...
function demoBlock(name: string) {
  return { kind: "interactive-demo" as const, identity: { id: "demo", version: "1.0.0" }, hook: "cms/editor-block/resolve" as const, manifestHash: digest("demo"), source: { html: `<button>${name}</button>`, css: "button{color:red}", javascript: "document.body.dataset.ready='yes'" }, staticFallback: `${name} 的替代內容` };
}
