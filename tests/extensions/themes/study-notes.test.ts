...
const stylesheet = readFileSync(path.resolve(import.meta.dirname, "../../../extensions/themes/study-notes/style.css"));
  const payload = {
    ...
    entries: [{ entryId: "guide", revisionId: "r1", content: { contract: "site-content/v1" as const, title: "公開筆記示範", blocks: blocks ?? [{ kind: "article" as const, text: "第一段。\n\n第二段。" }, { kind: "interactive-demo" as const, pluginIdentity: { id: "demo", version: "1.0.0", hookContract: "plugin-hooks/v1" as const, manifestHash: digest("demo") }, source: { html: "<button>執行</button>", css: "button{color:red}", javascript: "document.body.dataset.ready='yes'" }, staticFallback: "<strong>替代內容</strong>" }], seo: {} }, contentDigest: digest("content") }],
    routes: [{ route: "/guide", entryId: "guide", revisionId: "r1" }], media: [],
    seo: { pageContributions: [], siteContributions: [], evidence: [], omissionCount: 0 },
    theme: { identity: { id: "study-notes", version: "1.0.0", rendererContract: "theme-renderer/v1" as const, manifestHash: digest("manifest") }, entrySourceBase64: Buffer.from(source).toString("base64"), entryDigest: digest(source), resources: [{ file: "style.css", bytesBase64: Buffer.from(stylesheet).toString("base64"), digest: sha256Digest(stylesheet) }] }, plugins: [],
  };
...
test("預設 Theme 產生子路徑安全且可存取的完整公開頁面", async () => {
  ...
  const stylesheetFile = rendered.value.files.find((file) => file.path.startsWith("assets/theme/") && file.path.endsWith(".css"));
  assert.match(new TextDecoder().decode(stylesheetFile?.bytes), /:focus-visible/u);
});
