...
name: categorize
description: >-
  Resolve `category` and `tags` for notes in src/content/blog/ using the closed
  vocabulary in docs/content-taxonomy.md. Tag-as-truth — Subject dimension
  determines category via deterministic priority. Author wins — never overwrites
  manually set values; only fills empty fields.
targets:
  - '*'
---
# Categorize Skill
