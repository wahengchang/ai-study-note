...
Reach for it by hand for everything else an agent reads: your docs, specs and tickets, system and AFK prompts. The test is one question: does an agent read this? And it does not matter how the document gets in front of you, whether a pointer names it, a human pastes it, or it simply sits in the repo. For working out what a codebase actually contains in the first place, use grill-with-docs; this reference governs how a document reads, not what it knows.
...
It is this skill, renamed in v1.1. Practitioners were already pointing it at `AGENTS.md`, `CLAUDE.md`, docs, specs, tickets and runtime prompts long before the name caught up; structure, leading words and pruning turn out to be the craft of any text an agent reads. There is no alias. Reinstall under the new name.
