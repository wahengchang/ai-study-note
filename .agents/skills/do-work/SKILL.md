...
涉及 core module 時，實作前的計畫必須依 `.rulesync/subagents/` 的責任邊界，安排兩個職責不同且均非 owner 的專案子代理交叉審查（light review，指出明顯問題）；`.codex/agents/` 僅為 generated view。不得以 platform reviewer 或 `/ai-x` 的跨模型第二意見替代。
