...
- Before mutation, SiteDefinition canonicalizes and hashes both graph snapshots. It computes collision and impact across every claim that remains after the proposed target-graph change, not merely the directly edited claim. `route-claim-replacement-proposal/v1` replaces an existing target-graph claim's route and/or source revision; its impact is the complete after-state retained set.
- Replacement impact uses `route-move` when canonical route changes, `attribution-only` when only source revision changes, and `retained` for every other active claim. It is sorted by graph (`current`, then `published`), canonical `to`, then owner with locale-independent code-unit comparison.
- A successful mutation updates the target graph claim set atomically with its caller's configured-store active transaction. Validation and apply bind both graph digests; apply rechecks them before write and verifies target resulting/non-target baseline digests plus replacement impact after write. The resulting hash for each graph must be reproducible from that graph's canonical snapshot. On rollback, both graph hashes equal their pre-command values.
