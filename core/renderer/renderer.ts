...
function seo(input: RendererInput): Readonly<{ pages: ReadonlyMap<string, SeoPage>; siteUrl?: string; count: number; digest: Digest }> | null {
  ...
  const evidence = canonicalJsonBytes({ pages: snapshot.pages, ...(siteUrl === undefined ? {} : { publicSiteUrl: siteUrl }) });
  return !evidence.ok ? null : Object.freeze({ pages, ...(siteUrl === undefined ? {} : { siteUrl }), count: pages.size + (siteUrl === undefined ? 0 : 1), digest: sha256Digest(evidence.value) });
}
