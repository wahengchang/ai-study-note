...
export type ThemeRenderInput = Readonly<{ contract: "theme-render-input/v1"; selection: RendererInputV1["selection"]; entries: readonly Readonly<{ entryId: string; revisionId: string; content: RendererInputV1["entries"][number]["content"]; contentDigest: Digest; blocks: readonly string[] }>[]; routes: RendererInputV1["routes"]; media: RendererInputV1["media"]; resources: RendererInputV1["theme"]["resources"] }>;
...
export type RendererOutput = Readonly<{ contract: "renderer-output/v1"; rendererInputDigest: Digest; provenance: Readonly<{ publishedRevisionIds: readonly Readonly<{ entryId: string; revisionId: string }>[]; routeGraphDigest: Digest; mediaSelectionDigest: Digest; theme: Readonly<{ id: string; version: string; manifestHash: Digest }>; plugins: readonly Readonly<{ id: string; version: string; manifestHash: Digest }>[] }>; routes: readonly Readonly<{ route: string; filePath: string }>[]; seo: Readonly<{ evidenceDigest: Digest; omissionCount: number }>; files: readonly RenderedFile[]; outputDigest: Digest }>;
export type RendererFailure = Readonly<{ code: "INVALID_RENDERER_INPUT" | "RENDERER_INPUT_DIGEST_MISMATCH" | "RENDERER_MODULE_INVALID" | "RENDERER_CALLBACK_FAILED" | "RENDERER_CALLBACK_RESULT_INVALID" | "RENDER_OUTPUT_CONFLICT" | "SEO_CONTRIBUTION_CONFLICT" | "UNSUPPORTED_EXTENSION_CONTRACT"; owner: "Renderer"; subjectIds: readonly string[]; remediation: MessageRemediation }>;
