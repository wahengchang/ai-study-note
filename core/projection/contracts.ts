...
import type { PluginHost, PublicSeoEvidence } from "../plugin-host/index.js";
import type { SiteDefinition } from "../site-definition/index.js";
import type { ThemeHost } from "../theme-host/index.js";

export type ResolvedPageSeoContribution = Readonly<{ evidence: PublicSeoEvidence; contribution: Readonly<{ contract: "public-seo-page-contribution/v1"; entryId: string; revisionId: string; route: string; title: string; description?: string; canonicalUrl: string; openGraph: Readonly<{ title: string; description?: string; url: string; type: "article" }>; jsonLd: Readonly<{ type: "WebPage"; name: string; description?: string; url: string }> }> }>;
export type ResolvedSiteSeoContribution = Readonly<{ evidence: PublicSeoEvidence; contribution: Readonly<{ contract: "public-seo-site-contribution/v1"; sitemapUrls: readonly string[]; robots: Readonly<{ indexing: "allow" | "disallow"; sitemapUrl: string }> }> }>;
...
export type RendererInputV1 = Readonly<{
  ...
  seo: Readonly<{ pageContributions: readonly ResolvedPageSeoContribution[]; siteContributions: readonly ResolvedSiteSeoContribution[]; evidence: readonly PublicSeoEvidence[]; omissionCount: number; omissionDigest?: Digest }>;
}>;
...
export type CreateProjectionInput = Readonly<{ persistence: PersistenceStore; siteDefinition: SiteDefinition; dataMedia: DataMedia; contentReadModel: ContentReadModel; themeHost: ThemeHost; pluginHost: PluginHost }>;
