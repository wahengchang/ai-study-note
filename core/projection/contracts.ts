...
export type PublishedProjectionResult = Readonly<{ artifact: RendererInputArtifact; diagnostics: readonly PluginHostFailure[] }>;
export type PreviewInputArtifact = Readonly<{ bytes: Uint8Array; previewDigest: Digest; bytesDigest: Digest }>;
export type RendererTaxonomyBinding = Readonly<{ taxonomyId: string; termId: string; evidence: Readonly<{ taxonomyId: string; termId: string; label: string; slug: string; order: number }>; evidenceDigest: Digest }>;
export type RendererEntry = Readonly<{ entryId: string; revisionId: string; schemaIdentity: Readonly<{ schemaId: string; version: number }>; content: StructuredContent; contentDigest: Digest; taxonomyBindings: readonly RendererTaxonomyBinding[] }>;
...
export type RendererMediaAsset = Readonly<{ identity: Readonly<{ assetId: string; assetVersionId: string }>; objectDigest: Digest; byteLength: number; metadata: JsonValue; metadataDigest: Digest }>;
...
export type RendererTheme = Readonly<{ identity: ThemeIdentity; manifest: ThemeManifestV1; files: readonly RendererThemeFile[]; activationStateDigest: Digest }>;
export type RendererPluginRenderer = Readonly<{ identity: PluginActivationIdentity; manifest: PluginManifestV1; entryBytesBase64url: string; entryDigest: Digest; resources: readonly Readonly<{ file: string; bytesBase64url: string; digest: Digest }>[]; callbacks: readonly Readonly<{ hook: PluginPublicHookId; exportName: string; priority: number }>[] }>;
export type RendererSeoPage = Readonly<{ route: string; title?: string; description?: string; canonicalUrl: string; jsonLd?: JsonValue }>;
export type RendererSeo = Readonly<{ status: "available"; pages: readonly RendererSeoPage[]; publicSiteUrl?: string; indexing?: "allow" | "disallow"; omissionDigest: Digest }> | Readonly<{ status: "omitted"; pages: readonly never[]; omissionDigest: Digest }>;
...
export type RendererPlugins = Readonly<{ activationStateDigest: Digest; settingsStateDigest?: Digest; identities: readonly PluginActivationIdentity[]; renderers: readonly RendererPluginRenderer[]; seo: RendererSeo }>;
export type RendererInput = Readonly<{ contract: "renderer-input/v1"; inputDigest: Digest; selection: Readonly<{ publishedRevisionIds: readonly Readonly<{ entryId: string; revisionId: string }>[]; routeGraphDigest: Digest; mediaSelectionDigest: Digest; taxonomySelectionDigest: Digest }>; entries: readonly RendererEntry[]; routes: RendererRoutes; media: RendererMedia; theme: RendererTheme; plugins: RendererPlugins }>;
