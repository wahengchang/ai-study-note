...
import { isSiteContentSchemaIdentity } from "../content/index.js";
...
function taxonomyBindingOrder(left: RendererTaxonomyBinding, right: RendererTaxonomyBinding): number {
  return left.evidence.order - right.evidence.order || compare(left.taxonomyId, right.taxonomyId) || compare(left.termId, right.termId);
}
function validTaxonomyEvidence(value: unknown): value is RendererTaxonomyBinding["evidence"] {
  if (!exact(value, ["taxonomyId", "termId", "label", "slug", "order"])) return false;
  const { taxonomyId, termId, label, slug, order } = value;
  return typeof taxonomyId === "string" && taxonomyId.length > 0
    && typeof termId === "string" && termId.length > 0
    && typeof label === "string" && label.length > 0
    && typeof slug === "string" && /^[a-z0-9]+(?:-[a-z0-9]+)*$/u.test(slug)
    && Number.isSafeInteger(order);
}
function capturedTaxonomyBindings(value: unknown): readonly RendererTaxonomyBinding[] | null {
  if (!Array.isArray(value)) return null;
  const bindings: RendererTaxonomyBinding[] = [];
  for (const binding of value) {
    if (!exact(binding, ["taxonomyId", "termId", "evidence", "evidenceDigest"])) return null;
    const { taxonomyId, termId, evidence, evidenceDigest } = binding;
    if (typeof taxonomyId !== "string" || taxonomyId.length === 0
      || typeof termId !== "string" || termId.length === 0
      || typeof evidenceDigest !== "string" || !isDigest(evidenceDigest)
      || !validTaxonomyEvidence(evidence)
      || evidence.taxonomyId !== taxonomyId || evidence.termId !== termId) return null;
    const bytes = canonicalJsonBytes(evidence);
    if (!bytes.ok || sha256Digest(bytes.value) !== evidenceDigest) return null;
  ...
  if (bindings.some((binding, index) => {
    ...
    return previous !== undefined
      && (taxonomyBindingOrder(previous, binding) >= 0
        || previous.taxonomyId === binding.taxonomyId && previous.evidence.order === binding.evidence.order);
  })) return null;
  return Object.freeze(bindings);
}
function entry(record: RevisionRecord, dependencies: ProjectionDependencies, taxonomyBindings: readonly RendererTaxonomyBinding[]): ProjectionResult<RendererEntry> {
  if (!isSiteContentSchemaIdentity(record.schemaIdentity)) return { ok: false, error: projectionFailure("INVALID_REVISION_EVIDENCE", [record.identity.entryId, record.identity.revisionId]) };
  ...
  return { ok: true, value: Object.freeze({ entryId: record.identity.entryId, revisionId: record.identity.revisionId, schemaIdentity: Object.freeze({ ...record.schemaIdentity }), content: content.value.content, contentDigest: content.value.digest, taxonomyBindings }) };
}
...
function captureInSnapshot(input: Readonly<{ dependencies: ProjectionDependencies; mode: "current" | "published"; subject?: string }>, snapshot: PersistenceReadSnapshot): ProjectionResult<Captured> {
  ...
  for (const identity of selected) {
    ...
    const bindings = snapshot.getRevisionTaxonomyBindings(identity);
    const taxonomyBindings = bindings.ok ? capturedTaxonomyBindings(bindings.value) : null;
    if (taxonomyBindings === null) return { ok: false, error: projectionFailure("INVALID_REVISION_EVIDENCE", [identity.entryId, identity.revisionId]) };
    const value = entry(revision.value, input.dependencies, taxonomyBindings);
  ...
  const bare = {
    ...
    claims: Object.freeze(routes.value.claims.map((value) => Object.freeze({ ...value }))),
    references: Object.freeze(references),
    assets: Object.freeze([...assets.values()].sort((left, right) => compare(assetKey(left.identity), assetKey(right.identity)))),
    routeGraphDigest: routes.value.digest,
  };
  const bytes = canonicalJsonBytes(guardBody(bare));
  return !bytes.ok ? { ok: false, error: projectionFailure("PROJECTION_ENCODING_FAILED") } : { ok: true, value: Object.freeze({ ...bare, guard: sha256Digest(bytes.value) }) };
}
function published(captured: Captured): JsonValue {
  ...
  return freeze({
    contract: "published-projection/v1",
    entries: captured.entries.map((entry) => ({
      entryId: entry.entryId,
      revisionId: entry.revisionId,
      schemaIdentity: entry.schemaIdentity,
      route: routeByOwner.get(`${entry.entryId}\u0000${entry.revisionId}`),
...
function optionalSeoText(value: unknown): value is string | undefined { return value === undefined || seoText(value); }
function optionalJsonValue(value: unknown): value is JsonValue | undefined { return value === undefined || canonicalJsonBytes(value).ok; }
function publicSeo(snapshot: PublicSeoSnapshot, dependencies: ProjectionDependencies): ProjectionResult<RendererSeo> {
  ...
  for (const item of snapshot.page) {
    ...
    for (const candidate of contribution.pages) {
      if (!exact(candidate, ["route"], ["title", "description", "canonicalPath", "jsonLd"])) return { ok: false, error: projectionFailure("PROJECTION_ENCODING_FAILED") };
      ...
      const canonicalPath = Object.hasOwn(candidate, "canonicalPath") ? candidate.canonicalPath : route;
      const jsonLd = Object.hasOwn(candidate, "jsonLd") ? candidate.jsonLd : undefined;
      if (!seoText(route) || !optionalSeoText(title) || !optionalSeoText(description) || !seoText(canonicalPath) || !optionalJsonValue(jsonLd) || publicSiteUrl === undefined) return { ok: false, error: projectionFailure("PROJECTION_ENCODING_FAILED") };
      ...
      const page: RendererSeoPage = Object.freeze({ route, ...(title === undefined ? {} : { title }), ...(description === undefined ? {} : { description }), canonicalUrl: canonical.value, ...(jsonLd === undefined ? {} : { jsonLd }) });
      pages.push(page);
    }
  }
...
export function createProjectionPreview(dependencies: ProjectionDependencies): ProjectionPreview {
  return Object.freeze({
    ...
    async preview(input: Readonly<{ selection: "current" | "published"; subject: Readonly<{ entryId: string }> }>): Promise<ProjectionResult<PreviewInputArtifact>> {
      ...
      const mediaDigest = mediaSelectionDigest(materialized.value.media);
      const taxonomyDigest = taxonomySelectionDigest(first.value.entries);
      if (route === undefined || mediaDigest === null || taxonomyDigest === null) return { ok: false, error: projectionFailure(route === undefined ? "UNRESOLVED_ROUTE_REFERENCE" : "PROJECTION_ENCODING_FAILED") };
      ...
      return previewArtifact({ contract: "preview-input/v1", subject: Object.freeze({ entryId: input.subject.entryId }), selection: Object.freeze({ mode: input.selection, selectedRevision: first.value.selected[0]!, routeSelectionDigest: routeSelection, mediaSelectionDigest: mediaDigest, taxonomySelectionDigest: taxonomyDigest }), entry: first.value.entries[0]!, route: Object.freeze({ normalizedRoute: route.normalizedRoute, owner: route.owner, sourceRevisionId: route.sourceRevisionId }), media: materialized.value.media, theme: materialized.value.theme, plugins: previewPlugins(snapshot.value) });
    },
  });
}
