...
/** `optional` 只放寬「可缺席」，不放寬形狀：出現的 optional key 仍必須是 data property，未列出的 key 一律拒絕。 */
export function exact(value: unknown, keys: readonly string[], optional: readonly string[] = []): value is Record<string, unknown> {
  try {
    ...
    return Object.keys(descriptors).every((key) => (keys.includes(key) || optional.includes(key)) && "value" in descriptors[key]!)
