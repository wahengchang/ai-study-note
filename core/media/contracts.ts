...
export interface DataMediaPersistence { createMediaImportIntent(input: MediaImportIntent): DataMediaPortResult<MediaImportIntent>; commitReadyAssetVersion(input: MediaImportIntent): DataMediaPortResult<ReadyAssetVersion>; getReadyAssetVersion(identity: AssetVersionIdentity): DataMediaPortResult<ReadyAssetVersion>; getEntryPointers(entryId: string): DataMediaPortResult<Readonly<{ entryId: string; currentRevisionId: string; publishedRevisionId?: string }>>; getRevisionReferences(revision: Readonly<{ entryId: string; revisionId: string }>): DataMediaPortResult<readonly Readonly<{ assetVersion: AssetVersionIdentity }>[]>; }
...
export interface DataMedia { importLocal(input: ImportLocalMediaInput): DataMediaResult<ReadyAssetVersion>; getReadyAssetVersion(identity: AssetVersionIdentity): DataMediaResult<ReadyAssetVersion>; requireReadyAssetVersions(identities: readonly AssetVersionIdentity[]): DataMediaResult<readonly ReadyAssetVersion[]>; resolvePublishedSelection(entryId: string): DataMediaResult<PublishedMediaSelection>; }
