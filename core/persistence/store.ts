...
export function createPersistenceStore(database: SqliteAdapter): PersistenceStore {
  ...
  const runReadSnapshot = <T, E>(operation: (snapshot: PersistenceReadSnapshot) => TransactionDecision<T, E>): TransactionDecision<T, E | PersistenceFailure> => {
    ...
    const snapshot: PersistenceReadSnapshot = Object.freeze({
      getSchemaVersion(identity: SchemaVersionIdentity) { return record(() => transaction.getSchemaVersion(identity)); },
      getLatestSchemaVersion(schemaId: string) { return record(() => transaction.getLatestSchemaVersion(schemaId)); },
      listLatestSchemaVersions() { return record(() => transaction.listLatestSchemaVersions()); },
      getEntryPointers(entryId: string) { return record(() => transaction.getEntryPointers(entryId)); },
      listEntryPointers() { return record(() => transaction.listEntryPointers()); },
      listEntryRevisions(entryId: string) { return record(() => transaction.listEntryRevisions(entryId)); },
...
const ASSET_VERSION_SQL = "SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v LEFT JOIN media_objects o ON o.object_digest=v.object_digest LEFT JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?";
function createOperations(database: SqliteAdapter, live: () => boolean = () => true, markFailure?: (failure: PersistenceFailure) => void): PersistenceTransaction {
  ...
  return {
    ...
    getLatestSchemaVersion(schemaId) { return reading(() => {
      if (!validText(schemaId)) return refused("INVALID_PERSISTENCE_INPUT");
      const row = database.get("SELECT version,schema_bytes,schema_digest FROM schema_versions WHERE schema_id=? ORDER BY version DESC LIMIT 1", schemaId);
      const version = row === undefined ? null : positive(row.version); const bytes = row === undefined ? null : byte(row, "schema_bytes"); const digest = row === undefined ? null : digestField(row, "schema_digest");
      if (row === undefined) return refused("SCHEMA_VERSION_NOT_FOUND"); if (version === null || bytes === null || digest === null) return refused("STORAGE_FAILURE");
      return { ok: true, value: schemaRecord({ schemaId, version }, bytes, digest) };
    }); },
    listLatestSchemaVersions() { return reading(() => {
      const values: SchemaVersionRecord[] = [];
      for (const row of database.all("SELECT s.schema_id,s.version,s.schema_bytes,s.schema_digest FROM schema_versions s JOIN (SELECT schema_id,max(version) AS version FROM schema_versions GROUP BY schema_id) latest ON latest.schema_id=s.schema_id AND latest.version=s.version")) {
        ...
        values.push(schemaRecord({ schemaId, version }, bytes, digest));
      }
      values.sort((left, right) => compareCodeUnits(left.identity.schemaId, right.identity.schemaId) || left.identity.version - right.identity.version);
    ...
    listEntryPointers() { return reading(() => {
      const values: EntryPointerRecord[] = [];
      for (const row of database.all("SELECT entry_id,current_revision_id,published_revision_id FROM entry_pointers")) {
        ...
        if (entryId === null) return refused("STORAGE_FAILURE");
        const value = pointer(row, entryId, refused); if (!value.ok) return value;
        values.push(value.value);
      }
      values.sort((left, right) => compareCodeUnits(left.entryId, right.entryId));
      return { ok: true, value: Object.freeze(values) };
    }); },
    listEntryRevisions(entryId) { return reading(() => {
      if (!validText(entryId)) return refused("INVALID_PERSISTENCE_INPUT");
      const values: RevisionRecord[] = [];
      for (const row of database.all("SELECT r.revision_id,r.schema_id,r.schema_version,r.content_bytes,r.content_digest,r.restored_from_revision_id,l.operation_id,l.operation_kind FROM revisions r JOIN operation_lineage l ON l.entry_id=r.entry_id AND l.revision_id=r.revision_id AND l.creates_revision=1 WHERE r.entry_id=?", entryId)) {
        ...
        values.push(revisionRecord({ identity: { entryId, revisionId }, schemaIdentity: { schemaId, version }, contentBytes: bytes, contentDigest: digest, ...(restored === null ? {} : { restoredFromRevisionId: restored }), lineage: { operationId, operationKind } }, bytes, digest));
      }
      values.sort((left, right) => compareCodeUnits(left.identity.revisionId, right.identity.revisionId));
      return { ok: true, value: Object.freeze(values) };
    }); },
    ...
    createMediaImportIntent(input) { return guarded(() => {
      const normalized=normalizedIntent(input,failed); if(!normalized.ok)return normalized; const prior=intentRow(database.get("SELECT * FROM media_import_intents WHERE import_id=?",input.importId),refused);
      if(prior.ok)return sameIntent(prior.value,normalized.value)?{ok:true,value:copyIntent(normalized.value)}:failed("MEDIA_IMPORT_CONFLICT");
    ...
    deleteMediaImportIntentExact(input) { return guarded(() => {
      ...
      database.run("DELETE FROM media_import_intents WHERE import_id=?",normalized.value.importId); return {ok:true,value:undefined};
    }); },
    commitReadyAssetVersion(input) { return guarded(() => {
      const normalized=normalizedIntent(input,failed); if(!normalized.ok)return normalized; const intent=intentRow(database.get("SELECT * FROM media_import_intents WHERE import_id=?",normalized.value.importId),failed); if(!intent.ok)return intent; if(!sameIntent(intent.value,normalized.value))return failed("MEDIA_IMPORT_CONFLICT");
      const existingRow=database.get(ASSET_VERSION_SQL,normalized.value.identity.assetId,normalized.value.identity.assetVersionId);
      if(existingRow!==undefined){const existing=assetVersionRow(existingRow,normalized.value.identity,failed);if(!existing.ok)return existing;const version=existing.value;if(version.availability!=="ready"||version.objectDigest!==normalized.value.objectDigest||version.byteLength!==normalized.value.byteLength||version.metadataDigest!==normalized.value.metadataDigest||!sameBytes(version.metadataBytes,normalized.value.metadataBytes))return failed("MEDIA_IMPORT_CONFLICT");database.run("DELETE FROM media_import_intents WHERE import_id=?",normalized.value.importId);return{ok:true,value:readyFromIntent(normalized.value)};}
      const object=database.get("SELECT byte_length FROM media_objects WHERE object_digest=?",normalized.value.objectDigest); if(object===undefined)database.run("INSERT INTO media_objects (object_digest,byte_length) VALUES (?,?)",normalized.value.objectDigest,normalized.value.byteLength);else if(object.byte_length!==normalized.value.byteLength)return failed("MEDIA_IMPORT_CONFLICT");
...
function readMediaStartupSnapshot(database: SqliteAdapter): PersistenceResult<MediaStartupSnapshot> {
  const failed: Fail = (code) => persistenceResultFailure(code);
  try {
    return database.readTransaction(() => {
      const pendingIntents: MediaImportIntent[] = [];
      const imports = new Set<string>();
      for (const row of database.all("SELECT * FROM media_import_intents")) {
        const intent = intentRow(row, failed); if (!intent.ok) return intent;
        if (imports.has(intent.value.importId)) return failed("STORAGE_FAILURE");
        imports.add(intent.value.importId); pendingIntents.push(intent.value);
      }
      const assetVersions: AssetVersionRecord[] = [];
      const identities = new Set<string>();
      for (const row of database.all("SELECT v.asset_id,v.asset_version_id,v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability,m.asset_id AS media_asset_id FROM asset_versions v LEFT JOIN media_objects o ON o.object_digest=v.object_digest LEFT JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id LEFT JOIN media_assets m ON m.asset_id=v.asset_id")) {
        ...
        if (!version.ok || text(row, "media_asset_id") !== assetId) return failed("STORAGE_FAILURE");
        const key = version.value.identity.assetId + "\u0000" + version.value.identity.assetVersionId;
        if (identities.has(key)) return failed("STORAGE_FAILURE");
        identities.add(key); assetVersions.push(version.value);
      }
      const availability = new Set<string>();
      for (const row of database.all("SELECT asset_id,asset_version_id FROM asset_version_availability")) {
        const assetId = text(row, "asset_id"), assetVersionId = text(row, "asset_version_id");
        if (assetId === null || assetVersionId === null) return failed("STORAGE_FAILURE");
        const key = assetId + "\u0000" + assetVersionId;
        if (!identities.has(key) || availability.has(key)) return failed("STORAGE_FAILURE");
        availability.add(key);
      }
      if (availability.size !== identities.size) return failed("STORAGE_FAILURE");
      for (const row of database.all("SELECT asset_id,asset_version_id FROM revision_refs")) {
        const assetId = text(row, "asset_id"), assetVersionId = text(row, "asset_version_id");
        if (assetId === null || assetVersionId === null || !identities.has(assetId + "\u0000" + assetVersionId)) return failed("STORAGE_FAILURE");
      }
      pendingIntents.sort((a,b) => compareCodeUnits(a.importId,b.importId) || compareAssetVersions(a.identity,b.identity));
      assetVersions.sort((a,b) => compareAssetVersions(a.identity,b.identity));
...
function validAssetVersionAvailability(value:unknown):value is AssetVersionAvailability{return value==="ready"||value==="archived"||value==="missing";}
...
function normalizedIntent(input:MediaImportIntent,failed:Fail):PersistenceResult<MediaImportIntent>{if(!validIntent(input))return failed("INVALID_PERSISTENCE_INPUT");const canonical=validateCanonicalBytes(input.metadataBytes,input.metadataDigest);if(!canonical.ok)return failed(canonical.code);return{ok:true,value:{...input,identity:{...input.identity},metadataBytes:copyBytes(canonical.bytes),metadataDigest:canonical.digest}};}
