...
function createOperations(database: SqliteAdapter, live: () => boolean = () => true, markFailure?: (failure: PersistenceFailure) => void): PersistenceTransaction {
  ...
  return {
    ...
    commitReadyAssetVersion(input) { return guarded(() => {
      ...
      const existingRow=database.get("SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v LEFT JOIN media_objects o ON o.object_digest=v.object_digest LEFT JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?",normalized.value.identity.assetId,normalized.value.identity.assetVersionId);
    ...
    getAssetVersion(identity) { return reading(() => { if(!validAssetIdentity(identity))return refused("INVALID_PERSISTENCE_INPUT"); const row=database.get("SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v LEFT JOIN media_objects o ON o.object_digest=v.object_digest LEFT JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?",identity.assetId,identity.assetVersionId); if(row===undefined)return{ok:true,value:undefined}; return assetVersionRow(row,identity,refused); }); },
    getReadyAssetVersion(identity) { return reading(() => { if(!validAssetIdentity(identity))return refused("INVALID_PERSISTENCE_INPUT"); const value=assetVersionRow(database.get("SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v LEFT JOIN media_objects o ON o.object_digest=v.object_digest LEFT JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?",identity.assetId,identity.assetVersionId),identity,refused); if(!value.ok)return value; return value.value.availability==="ready"?{ok:true,value:{...value.value,availability:"ready"}}:refused("ASSET_VERSION_NOT_FOUND"); }); },
...
function readMediaStartupSnapshot(database: SqliteAdapter): PersistenceResult<MediaStartupSnapshot> {
  ...
  try {
    return database.readTransaction(() => {
      ...
      const assetVersions: MediaAssetVersionRecord[] = [];
...
function assetVersionRow(row:SqliteRow|undefined,identity:AssetVersionIdentity,failed:Fail):PersistenceResult<MediaAssetVersionRecord>{if(row===undefined)return failed("ASSET_VERSION_NOT_FOUND");const objectDigest=digestField(row,"object_digest"),byteLength=row.byte_length,metadataBytes=byte(row,"metadata_bytes"),metadataDigest=digestField(row,"metadata_digest"),availability=row.availability;if(!validAssetIdentity(identity)||objectDigest===null||!nonnegative(byteLength)||metadataBytes===null||metadataDigest===null||(availability!=="ready"&&availability!=="archived"&&availability!=="missing"))return failed("STORAGE_FAILURE");const canonical=validateCanonicalBytes(metadataBytes,metadataDigest);if(!canonical.ok)return failed(canonical.code);return{ok:true,value:{identity:{...identity},objectDigest,byteLength,metadataBytes:copyBytes(canonical.bytes),metadataDigest:canonical.digest,availability}};}
