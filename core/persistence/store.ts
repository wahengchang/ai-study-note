...
function createOperations(database: SqliteAdapter, live: () => boolean = () => true, markFailure?: (failure: PersistenceFailure) => void): PersistenceTransaction {
  ...
  return {
    ...
    getAssetVersion(identity) { return reading(() => assetVersionRow(database.get("SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v JOIN media_objects o ON o.object_digest=v.object_digest JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?", identity.assetId, identity.assetVersionId), identity, refused)); },
    setAssetVersionAvailability(identity, availability) { return guarded(() => { if (!validAssetVersionAvailability(availability)) return failed("INVALID_PERSISTENCE_INPUT"); const record = assetVersionRow(database.get("SELECT v.object_digest,o.byte_length,v.metadata_bytes,v.metadata_digest,a.availability FROM asset_versions v JOIN media_objects o ON o.object_digest=v.object_digest JOIN asset_version_availability a ON a.asset_id=v.asset_id AND a.asset_version_id=v.asset_version_id WHERE v.asset_id=? AND v.asset_version_id=?", identity.assetId, identity.assetVersionId), identity, failed); if (!record.ok) return record; database.run("UPDATE asset_version_availability SET availability=? WHERE asset_id=? AND asset_version_id=?", availability, identity.assetId, identity.assetVersionId); return { ok: true, value: { ...record.value, availability } }; }); },
    listPublishedAssetReferences(identity) { return reading(() => { if (!validAssetVersionIdentity(identity)) return refused("INVALID_PERSISTENCE_INPUT"); const values: PublishedAssetReference[] = []; for (const row of database.all("SELECT p.entry_id,p.published_revision_id FROM entry_pointers p JOIN revision_refs r ON r.entry_id=p.entry_id AND r.revision_id=p.published_revision_id WHERE p.published_revision_id IS NOT NULL AND r.asset_id=? AND r.asset_version_id=? ORDER BY p.entry_id,p.published_revision_id", identity.assetId, identity.assetVersionId)) { const entryId = text(row, "entry_id"), revisionId = text(row, "published_revision_id"); if (entryId === null || revisionId === null) return refused("STORAGE_FAILURE"); values.push({ entryId, revisionId, assetVersion: { ...identity } }); } return { ok: true, value: values }; }); },
...
function assetVersionRow(row:SqliteRow|undefined,identity:AssetVersionIdentity,failed:Fail):PersistenceResult<AssetVersionRecord>{if(row===undefined)return failed("ASSET_VERSION_NOT_FOUND");const objectDigest=digestField(row,"object_digest"),byteLength=row.byte_length,metadataBytes=byte(row,"metadata_bytes"),metadataDigest=digestField(row,"metadata_digest"),availability=row.availability;if(objectDigest===null||typeof byteLength!=="number"||metadataBytes===null||metadataDigest===null||!validAssetVersionAvailability(availability))return failed("STORAGE_FAILURE");return{ok:true,value:{identity:{...identity},objectDigest,byteLength,metadataBytes:copyBytes(metadataBytes),metadataDigest,availability}};}
...
function validAssetVersionIdentity(value:AssetVersionIdentity):boolean{return validText(value.assetId)&&validText(value.assetVersionId);}
