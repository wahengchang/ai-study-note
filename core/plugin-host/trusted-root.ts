...
export type TrustedPathLookup = Readonly<{ status: "available"; path: string }> | Readonly<{ status: "missing" }> | Readonly<{ status: "invalid" }>;
...
function missing(error: unknown): boolean {
  return typeof error === "object" && error !== null && "code" in error && ((error as { code?: unknown }).code === "ENOENT" || (error as { code?: unknown }).code === "ENOTDIR");
}
...
export async function validateTrustedRoots(input: Readonly<{ repositoryRoot: unknown; installedPluginsRoot: unknown }>): Promise<TrustedRoots | null> {
  ...
  const identity = await installedIdentity(installedPluginsRoot);
  return identity === null ? null : Object.freeze({ repositoryRoot, installedPluginsRoot, installedIdentity: identity });
}
...
export async function installedPluginDirectories(root: TrustedRoots): Promise<readonly string[]> {
  ...
  return Object.freeze(entries.filter((entry) => entry.isDirectory() || entry.isSymbolicLink()).map((entry) => entry.name).sort(compareCodeUnits));
}

export async function resolvePluginDirectory(root: TrustedRoots, pluginId: string): Promise<TrustedPathLookup> {
  if (pluginId.length === 0 || pluginId.includes("/") || pluginId.includes("\\") || pluginId.includes("\0")) return Object.freeze({ status: "invalid" });
  try {
    const resolved = await realpath(path.join(root.installedPluginsRoot, pluginId));
    if (!isInside(root.installedPluginsRoot, resolved)) return Object.freeze({ status: "invalid" });
    return (await stat(resolved)).isDirectory() ? Object.freeze({ status: "available", path: resolved }) : Object.freeze({ status: "missing" });
  } catch (error) {
    return Object.freeze({ status: missing(error) ? "missing" : "invalid" });
  }
}

export async function resolvePluginFile(pluginDirectory: string, file: string): Promise<TrustedPathLookup> {
  if (!isSafeRelativeFile(file)) return Object.freeze({ status: "invalid" });
  try {
    const resolved = await realpath(path.join(pluginDirectory, file));
    if (!isInside(pluginDirectory, resolved)) return Object.freeze({ status: "invalid" });
    return (await stat(resolved)).isFile() ? Object.freeze({ status: "available", path: resolved }) : Object.freeze({ status: "missing" });
  } catch (error) {
    return Object.freeze({ status: missing(error) ? "missing" : "invalid" });
  }
}
