...
export function createAuthoringReadFacade(input: CreateAuthoringReadFacadeInput): AuthoringReadFacade {
  ...
  return {
    ...
    async getContentType(request) { return stable(() => { if (!validId(request.schemaId)) return fail("INVALID_AUTHORING_READ_INPUT", [request.schemaId]); const records = input.persistence.listSchemaVersions(); if (!records.ok) return fail("AUTHORING_READ_FAILED"); const record = [...records.value].reverse().find((item) => item.identity.schemaId === request.schemaId); if (record === undefined) return fail("CONTENT_TYPE_NOT_FOUND", [request.schemaId]); const document = schemaDocument(record); return document === undefined ? fail("AUTHORING_READ_FAILED", [request.schemaId]) : { ok: true, value: document }; }); },
