...
| Persistence | migration ledger、schema version、revision、operation lineage，以及 schema migration 的 preflight／原子 execution durable lineage；`runReadSnapshot()` 提供 capability-safe 同 generation 唯讀讀取。 | [CMS-DB-01](../specs/cms-basic-contracts-v1/01-persistence-and-schema-migrations.md) | [public entry](../core/persistence/index.ts)；[read snapshot](../tests/core/persistence/read-snapshot.test.ts)、[revision store](../tests/core/persistence/revision-store.test.ts)。 |
| Plugin Host | durable exact-identity activation／deactivation／drift latch、CMS editor-block resolution、SaveRevision validator preparation／execution seam，以及 sealed public renderer snapshot 已實作；`resolveCmsEditorBlock()` 接受由 canonical revision 可完整重建的 `{id,version,hook,manifestHash}` binding，唯一 CMS production caller 是 Application 的 `resolveCurrentCmsEditorBlocks()`，browser 只讀其 exact DTO。`inspectActiveSnapshot()` 驗 exact evidence 但不寫入 drift latch。 | [CMS-CORE-05](../specs/cms-basic-contracts-v1/05-plugin-host-core.md) | [Plugin Host public entry](../core/plugin-host/index.ts)；[Plugin Host test](../tests/core/plugin-host/plugin-host.test.ts)、[Application resolution test](../tests/core/application/cms-editor-block-resolutions.test.ts)、[locale determinism](../tests/core/plugin-host/locale-determinism.test.ts) |
...
| Site Definition | current/published route normalization、claim proposal、configured-store active transaction-bound token，以及 route/source replacement 的完整 retained impact 已實作；`snapshotInReadSnapshot()` 重用 canonical route snapshot。 | [CMS-CORE-03](../specs/cms-basic-contracts-v1/03-route-graph-application-core.md) | [public entry](../core/site-definition/index.ts)；[read snapshot](../tests/core/site-definition/read-snapshot.test.ts)、[route replacement](../tests/core/site-definition/route-claim-replacement.test.ts) |
| DataMedia | local import 建立 ready asset version；`readReadyObject()` 在同一 descriptor 驗證並回傳 fresh exact bytes，其他 published selection／archive／restore 行為維持。 | [CMS-CORE-04](../specs/cms-basic-contracts-v1/04-media-lifecycle-application-core.md) | [public entry](../core/media/index.ts)；[verified object read](../tests/core/media/verified-object-read.test.ts)、[archive and restore](../tests/core/media/archive-restore-asset.test.ts) |
| External Authoring API | local credential lifecycle、fixed-origin server proof、Bearer authenticated `SaveRevision`／`PublishRevision` transport、`cms:save-revision` CLI、typed `LocalAuthoringClient` 與 browser ticket／memory-only session bootstrap 已實作。 | [implementation contract §7](../contracts/README.md#7-external-authoring-api-v1已核准runtime-部分實作) | [public entry](../apps/authoring-api/index.ts)；[credential lifecycle](../tests/apps/authoring-api/credential-lifecycle.test.ts)、[HTTP contract](../tests/apps/authoring-api/http-contract.test.ts)、[CMS session race](../tests/apps/authoring-api/cms-session.test.ts)、[credential CLI](../tests/apps/authoring-api/credential-cli.test.ts)、[SaveRevision CLI](../tests/apps/authoring-api/save-revision-cli.test.ts)。 |
| Theme Host | repository-external Theme identity、manifest/evidence 與 self-contained runtime import graph validation 已實作；不執行 runtime，沒有 activation；Renderer 只執行 Projection 封存後的 verified bytes。 | [implementation contract](../contracts/README.md) | [public entry](../core/theme-host/index.ts)；[Theme Host contract test](../tests/core/theme-host/theme-host.test.ts)。 |
| Projection + Preview | published-only `renderer-input/v1` artifact、single-subject current/published Preview、canonical parser 與 sandboxed preview document 已實作；Projection materializes exact Theme／Plugin bytes，不執行它們。 | [implementation contract](../contracts/README.md) | [public entry](../core/projection/index.ts)；[published isolation](../tests/core/projection/preview-isolation.test.ts)；[strict parse 與 capture 診斷](../tests/core/projection/strict-parse.test.ts)。 |
| Renderer + Delivery + Public UI／Release | static Renderer 僅接受 strict parsed artifact、Delivery 產生並重新驗證 immutable artifact、Public UI 只服務 verified snapshot；release 仍是 deferred boundary。 | [implementation contract](../contracts/README.md) | [Renderer](../core/renderer/index.ts)、[Delivery](../core/delivery/index.ts)、[Public UI](../apps/public-ui/index.ts)；[Renderer](../tests/core/renderer/renderer.test.ts)、[Delivery](../tests/core/delivery/delivery.test.ts)、[Public UI](../tests/apps/public-ui/server.test.ts)。 |
