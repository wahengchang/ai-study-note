...
| Content + Application | `SaveRevision` 在 canonical write 前以 real PluginHost snapshot 執行 validator；`PublishRevision` 重新驗證 selected current revision 的 schema、media 與 route proposal；`ReplaceMediaReference` 複製 current source 的完整 reference set 並只替換 ready target。三者只在其 target graph 原子更新 pointer／claim 與 lineage。 | [CMS-CORE-02](../specs/cms-basic-contracts-v1/02-content-lifecycle-application-core.md) | [public entry](../core/application/index.ts)；[SaveRevision](../tests/core/application/save-revision.test.ts)、[SaveRevision failures](../tests/core/application/save-revision-failures.test.ts)、[Plugin composition](../tests/core/application/save-revision-plugin-composition.test.ts)、[PublishRevision](../tests/core/application/publish-revision.test.ts)、[ReplaceMediaReference](../tests/core/application/replace-media-reference.test.ts) |
