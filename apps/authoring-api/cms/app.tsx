...
function ContentTypeList({ api }: Readonly<{ api: CmsApiClient }>): React.JSX.Element {
  ...
  const load = (): void => { const current = generation.next(); setError(undefined); void api.listContentTypes().then((next) => { if (current()) setCatalog(next); }).catch((reason: unknown) => { if (current()) setError(message(reason)); }); };
  useEffect(load, [api]);
...
function ContentTypeDetail({ api }: Readonly<{ api: CmsApiClient }>): React.JSX.Element {
  ...
  const generation = useLoadGeneration();
  const load = (): void => { const current = generation.next(); setItem(undefined); setError(undefined); void api.getContentType(schemaId).then((next) => { if (current()) setItem(next); }).catch((reason: unknown) => { if (current()) setError(message(reason)); }); };
...
function List({ api }: Readonly<{ api: CmsApiClient }>): React.JSX.Element {
  ...
  const generation = useLoadGeneration();
  const load = (): void => { const current = generation.next(); setError(undefined); void api.getEntries().then((next) => { if (current()) setCatalog(next); }).catch((reason: unknown) => { if (current()) setError(message(reason)); }); };
  useEffect(load, [api]);
  ...
  const items = catalog.items.filter((item) => filter === "all" || (filter === "draft" ? item.status === "draft" : item.status !== "draft")).sort((left, right) => titleCollator.compare(left.title, right.title) || left.entryId.localeCompare(right.entryId));
