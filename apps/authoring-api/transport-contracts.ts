...
const routeGraphSelectionSchema = z.enum(["current", "published"]);
const routeGraphDigestsSchema = z.object({ current: digestSchema, published: digestSchema }).strict();
const routeClaimSchema = z.object({ graph: routeGraphSelectionSchema, normalizedRoute: z.string(), owner: z.string(), sourceRevisionId: z.string() }).strict();
const routeChangeTargetSchema = z.object({ graph: routeGraphSelectionSchema, owner: z.string(), route: z.string(), sourceRevisionId: z.string() }).strict();
const routeImpactSchema = z.object({ change: z.enum(["route-move", "attribution-only", "retained"]), graph: routeGraphSelectionSchema, owner: z.string(), from: z.string(), to: z.string(), resultingSourceRevisionId: z.string() }).strict();
export const routeGraphSchema = z.object({ contract: z.literal("route-graph/v1"), normalization: z.literal("route-normalization/v1"), selection: routeGraphSelectionSchema, claims: z.array(routeClaimSchema), graphDigests: routeGraphDigestsSchema }).strict();
export const routeChangeProposalSchema = z.object({ contract: z.literal("route-change-proposal/v1"), baselineDigests: routeGraphDigestsSchema, target: routeChangeTargetSchema }).strict();
export const routeChangeCommandSchema = z.object({ contract: z.literal("route-change-command/v1"), operationId: z.string(), proposal: routeChangeProposalSchema }).strict();
export const routeChangeReceiptSchema = z.object({ contract: z.literal("route-change-receipt/v1"), claim: routeClaimSchema, impact: z.array(routeImpactSchema), baselineDigests: routeGraphDigestsSchema, resultingDigests: routeGraphDigestsSchema, stateDigest: digestSchema }).strict();
...
export type RouteGraphDto = Readonly<z.infer<typeof routeGraphSchema>>;
export type RouteChangeProposalDto = Readonly<z.infer<typeof routeChangeProposalSchema>>;
export type RouteChangeCommandDto = Readonly<z.infer<typeof routeChangeCommandSchema>>;
export type RouteChangeReceiptDto = Readonly<z.infer<typeof routeChangeReceiptSchema>>;
