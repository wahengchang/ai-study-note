...
import { createAuthoringReadFacade, createContentTypeAdministration, createDomainApplication, createPersistencePluginActivationStatePort } from "../../core/application/index.js";
...
export type StartCmsRuntimeInput = Readonly<{ repositoryRoot: string; databasePath: string; objectsRoot: string; installedPluginsRoot: string; installedThemesRoot: string; cmsAssetsRoot: string; themeIdentity: ThemeIdentity; credential: LocalAuthoringCredentialInput; logger: (event: AuthoringApiLogEvent) => void }>;
...
export async function startCmsRuntime(input: StartCmsRuntimeInput): Promise<CmsRuntimeResult<RunningCmsRuntime>> {
  if (!isAbsolute(input.repositoryRoot) || !isAbsolute(input.databasePath) || !isAbsolute(input.objectsRoot) || !isAbsolute(input.installedPluginsRoot) || !isAbsolute(input.installedThemesRoot) || !isAbsolute(input.cmsAssetsRoot) || typeof input.logger !== "function") return failure("INVALID_CMS_RUNTIME_INPUT");
  ...
  try {
    ...
    const objectStore = createLocalMediaObjectStore({ objectsRoot: input.objectsRoot });
    ...
    const pluginHost = await createPluginHost({ repositoryRoot, installedPluginsRoot: input.installedPluginsRoot, activationState: createPersistencePluginActivationStatePort({ persistence }) });
    ...
    const themeHost = await createThemeHost({ repositoryRoot, installedThemesRoot: input.installedThemesRoot });
    if (!themeHost.ok) return failure("CMS_THEME_HOST_UNAVAILABLE");
    const theme = await themeHost.value.resolveExact({ identity: input.themeIdentity });
    if (!theme.ok) return failure("CMS_THEME_HOST_UNAVAILABLE");
    ...
    const started = await startAuthoringApi({ domainApplication, credentialAuthority: credentials, cmsAssets: assets, logger: input.logger, authoringReadFacade, contentTypeAdministration, projectionPreview, themeIdentity: input.themeIdentity });
