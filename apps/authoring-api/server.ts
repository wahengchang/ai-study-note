...
function domainError(requestId: string, error: DomainApplicationFailure): Response {
  ...
  return specialized === undefined ? response({ contract: "authoring-error/v1", ...safe }, status) : response(specialized, status);
}
