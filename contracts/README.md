...
- CMS browser code moves to the sole `apps/cms/index.ts` entry. It owns Content Type catalog、initial version creation、current schema detail and catalog-derived version history, exclusively through authorized Authoring API endpoints, plus Plugin settings/activation and entry-editor SEO UI. Source-preserving Plugin editor-block UI normalized-document dirty/save bytes retain every canonical interactive block, while active Host output and safe Host diagnostics are derived display-only bytes. The editor has normalized-document dirty/save/analysis bytes, 400ms trailing analysis, stale-response suppression, conflict lock/reload focus, and accessible status. SEO unavailable is non-blocking; SEO never disables eligible Save/Publish.

### #308 contract — CMS document admission

CMS document routing is a finite exact allowlist: `GET /cms`, `GET /cms/`, `GET /cms/entries`, `GET /cms/entries/new`, `GET /cms/entries/:entryId`, `GET /cms/content-types`, `GET /cms/content-types/new`, `GET /cms/content-types/:schemaId`, and `GET /cms/plugins`. Built assets are admitted only from the built manifest allowlist. Every named CMS document route must be in both the CMS-document allowlist and the central logger's document route-template union; no `/cms/*`, prefix, history, SPA or wildcard fallback is permitted.
