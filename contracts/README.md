...
Before body parsing, every `/v1` route rejects Cookie and query. Local CMS browser and CLI requests may omit Bearer credentials; this local-only mode deliberately grants any process on the same machine access to CMS data and mutations. The listener must never bind beyond loopback.
