...
architecture checker scan whole repository `**/*.{ts,tsx}`、`**/*.sql`，排除 generated/read-only handoff roots；`.tsx` 與 `.ts` 同受 semantic-root、import、naming、parser/module-resolution 規則與 failing fixture 覆蓋。`apps/cms/index.ts` 是唯一 CMS public entry；CMS 使用 exact pinned React 19.2.8/React DOM 19.2.8、Vite 8.2.2 與相關 type/plugin pins，`cms:build` 產 manifest/content-hashed `/cms/assets/`，`cms:serve` 只 serve allowlist。Ajv 只作 app adapter 的 JSON Schema validation，Playwright 只作 private browser launcher 與可觀察驗證；core 維持 framework-agnostic。
...
`apps/authoring-api` 是唯一 localhost transport composition root；command/read HTTP 都只能經 `core/application/index.ts` public seam，唯一例外是 GitHub #289 owner 的 `POST /v1/preview` 可直接呼叫 Projection public seam。公開訪客只讀 immutable static artifact。listener 固定 `127.0.0.1:43127`／`http://127.0.0.1:43127`，拒絕 localhost、IPv6、alternative IP、forwarded headers、CORS、redirect。所有 response 有 no-store/no-cache/nosniff/no-referrer；除下列兩個成功 DTO field 外，所有 response、header、log、diagnostic 都先移 secret 再 redact `asn_v1_`／`asn_bt_v1_`。

唯二可含 secret canary 的 field 是 server-generated 且通過 strict success schema 的 `browser-ticket/v1.ticket` 與 `browser-session/v1.apiKey`；dedicated no-store writer 不走通用 redaction，其他任何 field 不得含 secret。CLI 經同一 keep-alive TCP socket proof 後，以 `browser-ticket-mint-request/v1 {generation,proofNonce}` mint `browser-ticket/v1 {ticket,generation,expiresInSeconds:60}`；proof grant 以 monotonic 5 秒存活，`now >= expiresAt` 即失效。raw fragment 只能是 `#asn_bt_v1_<43-char-unpadded-base64url>`；page 的第一個可執行動作必先 capture raw hash，再同步 `history.replaceState(null,"",location.pathname)` 清 query/hash，最後才 validate fragment 並 dynamic-import session/app。以 exact `browser-session-exchange/v1 {ticket}` 兌換 `browser-session/v1 {generation,apiKey}`；ticket 60 秒、single-use、generation-bound，consume 必在 response 前 delete，ticket/key 僅在 lexical/module closure，pagehide 清 key，reload/任何 authenticated 401 鎖定。

在任何 secret/auth/body parse 前，所有 `/_local`／`/v1` route 拒絕 Cookie/query。`POST /_local/server-proof` 只允許 absent Origin/no Bearer；`POST /_local/browser-tickets` 只允許 absent Origin、無 `Sec-Fetch-*`、exactly-one current Bearer、無 cookie/query 且 same proof-bound socket；`POST /_local/browser-session` 只允許 exact Origin、`Sec-Fetch-Site:same-origin`、無 Bearer/cookie/query。Save/Publish POST 保留 browser exact Origin+same-origin+Bearer 與 CLI absent Origin+no Fetch Metadata+Bearer profiles；`GET /v1/plugins`、`POST /v1/plugins/activate`、`POST /v1/plugins/settings`、`GET /v1/entries/:entryId/current`、`POST /v1/entries/:entryId/seo-analysis` 是 finite routes，path/body entry／identity ID 必 exact；前兩 Plugin mutation 與 SEO analysis POST 只允許 browser profile。Plugin/entry GET 只允許 exact Host+Bearer+`Sec-Fetch-Site:same-origin`、無 cookie/query/body，Origin 可缺席但 present 必 exact。browser fetch 固定 `credentials:"omit"`、`cache:"no-store"`、`redirect:"error"`、`referrerPolicy:"no-referrer"`；CMS shell/assets 永不 set cookie。CMS document/history `GET /cms`、`GET /cms/entries`、`GET /cms/entries/new`、`GET /cms/entries/:entryId` 只允許 mode `navigate`、dest `document`、site `none|same-origin`、無 Bearer/cookie/query，且 dynamic ID 為未 percent-encode 的單一 segment；assets 只允許 exact manifest key、same-origin、無 cookie/query 與正確 MIME/destination。未列 route、OPTIONS、wrong Host/Origin/forwarded、malformed/duplicate/old/revoked credential 都不得 command/canonical mutation，且 response/log/diagnostic 不得洩漏 canary。
