# Dispatch Brief — GbQd4

**Generated**: 2026-04-11
**Workflow**: [`cron/git-auto.md`](./git-auto.md)
**Purpose**: Audit the open issue queue on `wahengchang/ai-study-note`, filter for research- and writing-shaped items, analyze each item's operational requirements, and delegate to the owning agent in [`claude/agents/`](../claude/agents) so the next automation run can pick the correct issue per §4 of the workflow.
...
| #63 | Adopt Mintlify as new framework for ai-study-note docs site | framework migration | no — architecture decision, not a research/writing note |
| #67 | Fix duplicate article titles on ai-study-note pages | layout/bug | no — PR #68 open; not research/writing |
...
`#61` is in-shape but held by PR #73; per [`cron/git-auto.md`](./git-auto.md) §4 rule *"If issue already has an open PR, skip it."*
...
- **Support — `@content-ops` (Intake mode)**: walk [`docs/content-taxonomy.md`](../docs/content-taxonomy.md) to place the note. Likely destinations are `content/prompt-notes/` or an AI-workflow tooling folder. Stop-and-ask if ambiguous — never invent a folder or tag.
...
- **Support — `@diagram`**: one `flowchart LR` (4–6 nodes max) for the raw/wiki feedback loop. Apply the dark-theme `classDef` tokens from [`claude/agents/diagram.md`](../claude/agents/diagram.md); never `TD`.
...
- **Language**: zh-tw per repo convention (opening line of [`cron/git-auto.md`](./git-auto.md)).
- **Frontmatter**: every note requires `title`; `@content-ops` enforces the full `title` + `description` + `tags` schema from [`docs/content-taxonomy.md`](../docs/content-taxonomy.md).
- **Mermaid**: `direction LR` only; apply the dark-theme `classDef` tokens from [`claude/agents/diagram.md`](../claude/agents/diagram.md).
- **Build gate**: `@reviewer` blocks merge until `npm run quartz -- build` exits 0.
- **Branching**: each downstream implementation PR uses `auto/issue-<n>` branched fresh from `origin/main`, per [`cron/git-auto.md`](./git-auto.md) §4b. This dispatch PR itself implements no issue.
- **Staging discipline**: explicit-path staging only. Never `git add .`, never touch `.automation/`.

## Next run pickup

Per [`cron/git-auto.md`](./git-auto.md) §4 and §5, the next automation run must:
