...
async function getSatoriFontsSafe(
  ctx: BuildCtx,
): Promise<SatoriOptions["fonts"] | null> {
  ...
  try {
    ...
    if (fonts.length === 0) {
      console.warn(
        styleText(
          "yellow",
          "Warning: Skipping CustomOgImages because no fonts are available for OG generation.",
        ),
      );
      return null;
    }
    ...
    console.warn(
      styleText(
        "yellow",
        `Warning: Skipping CustomOgImages because font fetch failed (${message}).`,
      ),
    );
    return null;
  }
...
> = (userOpts) => {
  ...
  return {
    ...
    async *emit(ctx, content, _resources) {
      const fonts = await getSatoriFontsSafe(ctx);
      if (!fonts) return;
