...
- 補齊 #282 的 Authoring API admission/security proof：兩個 migration route 逐一驗 missing／malformed scheme／malformed key shape／duplicate／invalid／rotate 後的 old key／revoked key、cookie 與 query transport、evil Host／X-Forwarded-For／Forwarded、evil Origin 與缺 same-origin fetch metadata 的 exact origin、OPTIONS／GET、415、invalid JSON、route 自身 1 MiB bound remediation 與 percent-encoded schemaId 404；被拒絕的 request 一律零 migration facade 呼叫、零 canonical mutation，response 與 log 無 `asn_` canary。
- 未知 Content Type 於兩個 route 均回 404 `CONTENT_TYPE_NOT_FOUND`（owner `ContentTypeMigration`），unsafe credential store 回 503 且不進入 migration seam。
...
- `npm run typecheck`
- `node --import tsx --test tests/core/application/content-type-migration.test.ts tests/apps/authoring-api/http-contract.test.ts`：36/36 通過。
