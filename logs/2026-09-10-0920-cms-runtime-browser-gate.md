...
- `tests/apps/cms/runtime-browser-gate.test.ts`：真實 fixed-origin Chromium journey 驗證四條 route pathname、session bootstrap、heading focus／skip link／named main、iframe content 與 `sandbox=""`、tab wrap、dialog focus trap／Escape／status focus、375px 實際 Save 與無水平 overflow。
